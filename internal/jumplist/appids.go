...
// appIDDescriptions maps a jump list's file name to the application that wrote it.
//
// The name is a hash of the application, so it is the only thing identifying
// which program a jump list belongs to — nothing inside the file says so. There
// is no algorithm to invert it; the mapping is a table built by running
// applications one at a time and recording what they produced.
//
// Source: the AppIDs table of Eric Zimmerman's JumpList library
// (github.com/EricZimmerman/JumpList), MIT licensed. It already contains every
// one of the 346 ids in 4n6k's AppID master list, which was checked entry by
// entry rather than assumed.
//
// Coverage is partial and always will be: it named 37 of the 85 applications
// with a jump list on the host this was built against. An id the table does not
// have is reported as the hash, never as a guess.
var appIDDescriptions = map[string]string{
...
// AppIDDescription names the application a jump list belongs to, or "" when the
// table does not have it.
func AppIDDescription(appID string) string {
	return appIDDescriptions[appID]
}
