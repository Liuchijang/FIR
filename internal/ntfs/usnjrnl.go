...
func init() { collector.Register(&usnJrnlCollector{}) }
...
func (c *usnJrnlCollector) Collect(ctx context.Context, outputDir string) ([]collector.FileInfo, error) {
	log := logging.G()
	...
	return []collector.FileInfo{{Path: "$UsnJrnl_J", SHA256: hash, Size: totalWritten}}, nil
}
